...
#ifdef CONFIG_PM
static void enterprise_suspend_work(void)
{
...
static void enterprise_resume_work(void)
{
...
static int __init enterprise_gpio_rtc_init(void)
{
	int ret;
...
#ifdef CONFIG_PM
	tps_platform.suspend_work = enterprise_suspend_work;
	tps_platform.resume_work = enterprise_resume_work;
#endif
	return 0;
...
int __init enterprise_regulator_init(void)
{
	...
	i2c_register_board_info(4, enterprise_regulators,
		    ARRAY_SIZE(enterprise_regulators));
