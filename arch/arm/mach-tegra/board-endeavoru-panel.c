...
static struct regulator *enterprise_lcd_reg_r;
...
//	tegra_gpio_disable(LCM_TE);
	...
	if (enterprise_lcd_reg == NULL) {
		...
		if (enterprise_lcd_reg != NULL) {
			regulator_enable(enterprise_lcd_reg);
		}
	}

	if (enterprise_lcd_reg_r == NULL) {
		enterprise_lcd_reg_r = regulator_get(NULL, "v_lcm_3v3");
		enterprise_lcd_reg_r = NULL;
		if (enterprise_lcd_reg_r != NULL) {
			regulator_enable(enterprise_lcd_reg_r);
		}
	}
...
//	tegra_gpio_enable(LCM_TE);
	...
	if (enterprise_lcd_reg_r != NULL)
		regulator_disable(enterprise_lcd_reg_r);
