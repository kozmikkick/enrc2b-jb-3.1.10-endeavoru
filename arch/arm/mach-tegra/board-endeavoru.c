...
int plat_chip_asleep(struct kim_data_s *pdata)
{
...
int plat_chip_awake(struct kim_data_s *pdata)
{
...
static struct tfa9887_platform_data enterprise_speaker_amp_data = {
...
static struct i2c_board_info i2c_speaker_amp_devices[] = {
	{
		...
		.platform_data = &enterprise_speaker_amp_data,
		.irq = -1,
	},
};
#endif
...
static void __init enterprise_uart_init(void)
{
	...
	enterprise_bt_uart_pdata.bt_en = ENDEAVORU_GPIO_BT_SHUTDOWN_N;
	enterprise_bt_uart_pdata.bt_cts_irq = ENDEAVORU_GPIO_BT_HOST_WAKE;
...
static void __init tegra_endeavoru_init(void)
{
	...
	if (!is_tegra_debug_uartport_hs()) {
		headset_devices_xe[2] = NULL; /* do not register 1wire driver if debug enabled */
