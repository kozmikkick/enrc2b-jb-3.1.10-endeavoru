...
static int baseband_modem_power_on(struct baseband_power_platform_data *data)
{
	...
	auto_sleep(25); /* need 20 but 40 is more safe */
...
void baseband_xmm_set_power_status(unsigned int status)
{
	...
	case BBXMM_PS_L2:
		...
		if (short_autosuspend && (&usbdev->dev)) {
				pr_debug("autosuspend delay %lx ms,disable short_autosuspend\n", autosuspend_delay);
...
static void baseband_xmm_power_defaultsusp(struct work_struct *work)
{
	...
	pr_debug("%s set_autosuspend_delay <%lx>", __func__, autosuspend_delay);	
}
...
static void baseband_xmm_device_add_handler(struct usb_device *udev)
{
	...
	if (id) {
		...
		pr_info("enable autosuspend, timer <%lx>", autosuspend_delay);
	}
}
...
static int baseband_xmm_power_driver_probe(struct platform_device *device)
{
	...
	int err, ret=0;
	 ...
	 pr_info(MODULE_NAME"enum_delay_ms=%lx\n", enum_delay_ms);
...
static int __init baseband_xmm_power_init(void)
{
	...
	host_dbg_flag = 0;
	pr_info("%s - host_dbg_flag<0x%x>, modem_ver<0x%lx>, mfg_mode<%d>"
	...
	if( mfg_mode )
	{
		...
		pr_info("In MFG mode, autosuspend_delay <%lx>, short_autosuspend_delay <%d>"
