...
*/
	...
	//layout mapping for x,y axis
        if(accel->orientation[0] == 0){
		if(accel->orientation[1] == 1){
			if(accel->orientation[3] == 1){ //x:(0,1,0)y:(1,0,0)
				temp    = rbuf[0] ;
				rbuf[0] = rbuf[1] ;
				rbuf[1] = temp    ;
			}
			else{//x:(0,1,0)y:(-1,0,0)
				temp    = rbuf[0] ;
				rbuf[0] = rbuf[1] ;
				rbuf[1] = -temp	  ;
			}
		...
		if(accel->orientation[0] == 1) {
			if(accel->orientation[4] == 1){ //x:(1,0,0)y:(0,1,0)
                           // Do Nothing
			}
			else{//x:(1,0,0)y:(0,-1,0)
				rbuf[1] = -rbuf[1];
			}
...
continue_i2c:
	...
	if (res) {
		mpu_gyro_state = 1; /*represent gyro occur no ack*/
		dev_err(&this_client->adapter->dev,
