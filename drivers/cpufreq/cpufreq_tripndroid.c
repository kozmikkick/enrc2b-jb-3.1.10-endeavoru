...
#define MAX_SAMPLING_DOWN_FACTOR		(8)
#define TRANSITION_LATENCY_LIMIT		(10 * 1000 * 1000)
#define CPU_FREQ_SAMPLING_LATENCY_MULTIPLIER	(1000)
#define CPU_FREQ_MIN_TICKS			(10) /* we need at least 10 */

#define MIN_SAMPLING_RATE_RATIO			(2)
#define MIN_STAT_SAMPLING_RATE			(MIN_SAMPLING_RATE_RATIO * jiffies_to_usecs(CPU_FREQ_MIN_TICKS))
...
/* cpufreq_tripndroid governor tunables */
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		if (this_dbs_info->enable) /* stop when already enabled */
			break;
...
static void tripndroid_early_suspend(struct early_suspend *handler) {
	suspended = 1;
}

static void tripndroid_late_resume(struct early_suspend *handler) {
	suspended = 0;
}

static struct early_suspend tripndroid_power_suspend = {
	.suspend = tripndroid_early_suspend,
	.resume = tripndroid_late_resume,
	.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1,
};

static int __init cpufreq_gov_dbs_init(void)
{
	register_early_suspend(&tripndroid_power_suspend);
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	unregister_early_suspend(&tripndroid_power_suspend);
	cpufreq_unregister_governor(&cpufreq_gov_tripndroid);
}


MODULE_AUTHOR("TripNRaVeR <info@tripndroid.com>");
MODULE_DESCRIPTION("'cpufreq_tripndroid' - cpufreq governor for android, optimised for tegra3");
MODULE_LICENSE("GPL");
